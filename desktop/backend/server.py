...
#    input queue). Handles are reconciled to the set of watches actually on the bus each poll.
#
# One handle per PLUGGED watch, not just the selected one (2026-09-05, two watches at once):
# with an Ambit2 and an Ambit3 both connected, list_watches/hid_enumerate opens EVERY watch to
# read its product/serial strings on every /api/devices poll, so an UNSELECTED watch with no
# keep-alive still re-presented and chimed on each of those. Holding a handle for every watch
# on the bus keeps them all claimed. _KEEPALIVE maps a watch's USB serial -> its held handle.
...
def keepalive_sync():
    """Hold one idle HID handle open to EVERY Suunto watch currently on the bus, so none of them
    re-present (and chime) when a tool - or hid_enumerate itself, which opens each watch to read
...
class Handler(BaseHTTPRequestHandler):
    ...
    # --- route weather + climb colouring (from an uploaded GPX) -------------------------
    # Read-only: they colour/forecast a GPX the user drew in any planner and never touch the
    # watch. The offline BRouter route-planner + POI-DB search were removed 2026-08-31 (André:
    # online planners/Basecamp already draw routes; the real need was weather along a route -
    # see docs/offline-routing.md). track_color.py is pure computation; weather_route.py needs
    # network. Both are called via run_tool (not imported) to keep the one dispatch convention
    # and the frozen (--tool) path working like every other endpoint.

    def _handle_router_color(self, body):
        """Body: {"gpx": str  OR  "points": [{lat,lon,ele?}] (>=2), "step"?, "window"?, "hgt"?}.
        Colours a track by climb gradient - the Plan page hands it an uploaded GPX (drawn in any
        planner), so it accepts either the raw GPX text or an explicit point list. track_color.py
        parses GPX itself, so a GPX just goes to it via a temp file. Never touches the watch."""
        gpx = body.get("gpx")
        if gpx:
            self._send_json(200, self._color_gpx(gpx, body))
            return
        pts = body.get("points") or []
        if not isinstance(pts, list) or len(pts) < 2:
            self._send_json(400, {"error": '"gpx" or "points" (>=2 {lat,lon}) is required'})
            return
        self._send_json(200, self._color_points(pts, body))

    def _color_gpx(self, gpx, body):
        """Colour a raw GPX string by climb: hand it straight to track_color.py (which parses
        GPX) via a temp .gpx file, return its JSON. Mirrors _color_points for the point path."""
        with tempfile.NamedTemporaryFile("w", suffix=".gpx", delete=False) as f:
            f.write(gpx)
            path = f.name
        try:
    ...
    def _handle_device(self):
        ...
        if info is None:
            ...
            info = _parse_info(out) if code == 0 else None
        if info is None:
            keepalive_sync()  # reconcile handles (this or another watch may have unplugged)
            self._send_json(502, {"ok": False, "raw_output": out, "stderr": err})
            return
        ...
        # Hold one idle HID handle open to every watch on the bus so none of them re-present (and
        # chime) on a tool's - or hid_enumerate's - open/close. See keepalive_sync / _KEEPALIVE.
        # Already-held watches are a no-op, so the steady state opens nothing new each poll.
        keepalive_sync()
