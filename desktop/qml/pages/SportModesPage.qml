...
PageFlickable {
    id: root
    contentWidth: width
    contentHeight: (root.isLegacyWatch ? legacySportModeCard.height : (root.selectedMode ? detailColumn.height : listColumn.height)) + Theme.spacingLarge * 2
    clip: true
    ...
    // Ambit1/2 (legacy PMEM) have no SBEM CustomModes region CustomModesService reads; their
    // sport modes go through the legacy editor card below (/api/legacy/sport-modes) instead.
    readonly property bool isLegacyWatch: DeviceCapabilities.supportsLegacyArchive
    ...
    Card {
        ...
        visible: HomeViewModel.connected && !HomeViewModel.isKailash && root.isLegacyWatch
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.top: parent.top
        anchors.topMargin: Theme.spacingLarge
        width: 560
    ...
    Column {
        id: listColumn
        visible: !HomeViewModel.isKailash && !root.isLegacyWatch && DeviceCapabilities.supportsSportModes && !root.selectedMode
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.top: parent.top
        anchors.topMargin: Theme.spacingLarge
        width: 560
        ...
        Row {
            width: parent.width
            spacing: Theme.spacingSmall
            visible: DeviceCapabilities.supportsMultisport && root.deviceMaxMultisport > 0
            Text {
                anchors.verticalCenter: parent.verticalCenter
    ...
    Card {
        id: pendingBar
        visible: root.hasPendingEdits && !HomeViewModel.isKailash && !root.isLegacyWatch && root.selectedMode
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.top: parent.top
    ...
    Column {
        id: detailColumn
        visible: !HomeViewModel.isKailash && !root.isLegacyWatch && root.selectedMode
        anchors.horizontalCenter: parent.horizontalCenter
        ...
        Card {
            width: parent.width
            // Hidden on a watch whose displays this app cannot decode yet (the Ambit1) -
            // better an absent section than an empty one implying it has no screens.
            visible: root.selectedMode !== null && DeviceCapabilities.supportsSportModeDisplays
            Column {
                id: displaysColumn
                width: parent.width
                spacing: Theme.spacingMedium
