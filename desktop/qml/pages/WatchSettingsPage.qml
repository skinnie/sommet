...
PageFlickable {
    ...
    function autoRead() {
        // 2026-08-23: GET /api/settings now device-dispatches, serving the Ambit1's own
        // settings in this same schema (35 fields grouped General/Units/Personal), so this
        // page reads for every non-Garmin watch. The Ambit1's fields come back
        // writable:false - see the backend's own note on why that is honest rather than
        // cautious - and the shared field renderer already shows a non-writable row as
        // read-only, so nothing here needs a per-device branch.
